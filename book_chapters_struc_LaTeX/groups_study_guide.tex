...
\item
A \term{group} is a set $G$ with a binary operation $\circ$ that meets the requirements of closure, identity element, inverse element, and associativity. The group may be denoted by $(G,\circ)$, or just $G$ if the operation $\circ$ is clear from the context  (Definition~\ref{group_definition}, and remark following)
\item
