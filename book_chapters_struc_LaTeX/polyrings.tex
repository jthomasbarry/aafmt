...
\end{rem}
 ...
 \begin{example}\label{example:poly:poly3.5} Express the following polynomials in summation notation:
\begin{enumerate}[(a)]
\item
$p_1(x)=1 + x+x^2+x^3$
\item
...
\begin{exercise}\label{exercise:poly:poly3l}
...
\begin{exercise}\label{exercise:poly:mult2way}
...
\begin{exercise}\label{exercise:poly:multform}
\begin{enumerate}[(a)]
\item
...
\begin{exercise}\label{exercise:poly:poly7}
...
\begin{exercise}\label{exercise:poly:5Z}
\begin{enumerate}[(a)]
\item
...
\begin{exercise}\label{exercise:poly:poly8}
...
\begin{exercise}\label{exercise:poly:poly9}
...
%%%% \begin{exercise}\label{exercise:poly:polyCoef10}
%%%% \begin{enumerate}[(a)]
%%%% \item
...
\begin{prop}\label{proposition:poly:polyclosed} Given that $R$ satisfies conditions (I)-(V) listed above. Then Definitions~\ref{defsumpoly} and \ref{defprodpoly} produce  closed addition and multiplication operations in $R[x]$.
\end{prop}
...
\begin{prop}\label{proposition:poly:polysumcommuteassociate}  Given that $R$ satisfies conditions (I)-(V) listed above, then addition in $R[x]$ is both commutative: 
	\[p(x)+q(x) = q(x) + p(x), \]	
and associative:	
...
\begin{prop}\label{proposition:poly:polyadditiveidentity}  Given that $R$ satisfies properties (I)-(V), then the additive identity of $R[x]$ is $0x^0$, where $0$ denotes the additive identity of $R$. 
\end{prop}
...
\begin{prop}\label{proposition:poly:polyadditiveinverse} Let $p(x)=\sum_{i=0}^n a_i x^i$ be a polynomial in $R[x]$, where $R$ satisfies properties (I)-(V). Then the additive inverse of $p(x)$ is $q(x)=\sum_{i=0}^n (-a_i) x^i$, where $-a_i$ is the additive inverse of $a_i$ in $R$.
\end{prop}
...
\begin{prop}\label{proposition:poly:polyadditivegroup} Let $R[x]$ be the  set of polynomials over a set $R$ that satisfies properties (I)-(V). Then $R[x]$ is an abelian group under addition (recall that ``abelian'' means that the group's operation is commutative). 
\end{prop}
...
\begin{prop}\label{proposition:poly:polymultassociate} Multiplication in $R[x]$ is associative: 
...
\begin{prop}\label{proposition:poly:polyleftrightdistributivity} Polynomials in $R[x]$ have both right distributivity across addition:
...
%\begin{exercise}\label{exercise:poly:polymult}
...
\label{sec:PolynomialsAndDivision}
...
\begin{exercise}\label{exercise:poly:div1}
...
\begin{exercise}\label{exercise:poly:div2}
\begin{enumerate} [(a)]
...
\begin{example}\label{example:poly:euclideanpoly} 
...
\begin{example}\label{example:poly:euclideanZn} 
...
\begin{prop}\label{proposition:poly:polyremainder}
...
\begin{prop}\label{proposition:poly:polydivides}
...
Proposition~\ref{proposition:complex:nonzero_complex_product} of Chapter~\ref{complex}). Actually, this \term{zero-divisor property} hold for any set $R$ that satisfies (I)-(VII):

\begin{prop}\label{proposition:poly:noZeroDiv} Suppose that $F$ satisfies properties (I)-(VII). Given $a,b \in F$ and $ab = 0$, then either $a=0$ or $b=0$. 
\end{prop}
...
\begin{prop}\label{proposition:poly:polyremainder2}
...
\begin{prop}\label{proposition:poly:FTOA}(\emph{Fundamental Theorem of Algebra: easy part})
...
\begin{prop}\label{proposition:poly:atmostnroots}
...
\begin{exercise}\label{exercise:poly:roots}
\begin{enumerate}[(a)]
\item
...
\begin{example}\label{example:poly:poly_complex} 
...
\begin{prop}\label{proposition:poly:Rationalroots}
...
\begin{exercise}\label{exercise:poly:complexroots}
\begin{enumerate}[(a)]
\item
...
\begin{prop}\label{proposition:poly:FTOAhard}(\emph{Fundamental Theorem of Algebra: hard part})
...
\begin{example}\label{example:poly:poly_complexroots} 
...
\begin{example}\label{example:poly:poly_complexroots2} 
...
\begin{prop}\label{proposition:poly:complexlinear}
...
\begin{exercise}\label{exercise:poly:complexPoly}
