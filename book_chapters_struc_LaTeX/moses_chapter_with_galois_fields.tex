...
%%%%(c)
%%%%(c)
\chapter{Further topics in Cryptography}\label{Crypto}
...
\begin{exercise}\label{exercise:Crypto:k1isk2}\\
...
\begin{exercise}\label{exercise:Crypto:DLex1}
\begin{enumerate}[(a)]
\item
...
\begin{exercise}\label{exercise:Crypto:DLex2}
\begin{enumerate}[(a)]
\item
...
\begin{exercise}\label{exercise:Crypto:DLex3}
...
\begin{exercise}\label{exercise:Crypto:DLex4}
...
\begin{example}\label{example:Crypto:one-way}
...
\begin{exercise}\label{exercise:Crypto:DLP}
...
\end{figure}
\begin{example}\label{example:Crypto:ec:realplus}
...
\begin{exercise}\label{exercise:Crypto:root3}
...
\end{enumerate}
\end{example}
\begin{example}\label{example:Crypto:find P3} Given E: $y^2 = x^3 - x$, $P_1 = (2, -\sqrt{6})$, $P_2 = (4, \sqrt{24})$ from Figure~\ref{fig:DH:DHKE_16} above.
...
\begin{example}\label{example:Crypto:P1+P2} In this example we'll use arithmetic mod 17, so  all coefficients and variables take values on $\mathbb{Z}_{17}$.  
...
Therefore, $2P = (6,3)$.
\end{example}
\begin{exercise}\label{exercise:Crypto:P}
...
\begin{example}\label{example:Crypto:26} Moses will send the message, ``attack" using the code table above. The point $C$ is chosen as $C = (9,4)$.\\
...
\begin{exercise}\label{exercise:Crypto:A_2} Verify the value of $A_2$ shown in Example \ref{example:Crypto:26} above (show your calculation).
\end{exercise}
Moses lets $B = (11, 20)$ and $\beta$ $= 7$. These are his private keys. \\
	$B_1 = (11, 17)$, public key\\
	$B_2 = (23, 30)$, public key

\begin{exercise}\label{exercise:Crypto:B_2} Verify the value of $B_1$ and $B_2$ shown above (show your calculation).
\end{exercise}
\begin{flushleft}
...
\begin{prop}\label{proposition:Crypto:BezoutsIdentity}(\emph{Bezout's Identity})~ Let $x$ and $t$ be two-non-zero integers, and let $d$ be their GCD, then there exists integers $p$ and $a$ such that\\
