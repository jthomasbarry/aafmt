...
\item
Rewrite $(1-x)^{k+1}$ as $(1-x)^k \cdot (1-x)$. Then use $P(n)$  for $n=k$ to obtain an inequality. Using basic algebra, simplify the right-hand side until you obtain a quantity that is greater than $1-(k+1)x$.
\item
...
\]
(Use the fact from Chapter \ref{BeforeWeBegin} that $a\le b$ and $c\le d$ implies
$a+c\le b+d$.)
