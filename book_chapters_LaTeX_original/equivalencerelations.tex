...
\sectionvideohref{nAFxMRkIBNE&index=17&list=PL2uooHqQ6T7PW5na4EX8rQX2WvBBdM8Qo}} \label{sec.relation}
...
\begin{example}{bbal}
Let $P$ be the set of all professional basketball players in the NBA, and let $T$ be the set of NBA teams.  $f_T: P \to T$ as follows:
...
\section{Partitions and properties of binary relations  \quad\sectionvideohref{SFk--oSOzfg&list=PL2uooHqQ6T7PW5na4EX8rQX2WvBBdM8Qo&index=18}} \label{Partitions1}
...
\begin{defn} A \term{partition}\index{Partition} $\mathcal{P}$ of a set~$A$ is a collection of nonempty subsets of~$A$, such that each element of~$A$ is in exactly one of the subsets in $\mathcal{P}$. In other words:
\begin{enumerate}[(a)]
...
\begin{enumerate}[(i)]
\item Draw the real number line from $-5$ to 5, and indicate the sets $R_1, R_2 \in \mathcal{P}$ (similar to Figure~\ref{fig:partition_discrete}).
\item Graph the associated binary relation $\rel_\mathcal{P}$. You only need to graph from $-5$ to 5. \hyperref[secEqRelChapHints]{(*Hint*)}
\end{enumerate}

\item Let $\mathcal{P}=\{\ldots, R_{-2}, R_{-1}, R_0, R_1, R_2, \ldots \}$ where $R_n=\{x \mid x \in \mathbb{R}, \lfloor x \rfloor = n \}$ for any integer $n$. \footnote{We will use these brackets, $\lfloor~\rfloor$, to represent the \term{floor function}\index{Floor function}, also known as the greatest integer function. The floor function takes a real number, $x \in \mathbb{R}$ as input and outputs the greatest integer that is less than or equal to $x$.  Mathematically we can express this as:
$$\lfloor x \rfloor=\max \{ m\in \mathbb{Z} ~|~m\leq x \}.$$

For example: $\lfloor 4 \rfloor = 4$, $\lfloor \pi \rfloor = 3$, and $\lfloor -2.3 \rfloor = -3$.}
\begin{enumerate}[(i)]
...
\item Let $\mathcal{P}=\{R_E, R_O\}$ where $R_E=\{x \mid x \in \mathbb{R}, \lfloor x \rfloor \text{ is even}\}$ and $R_O=\{x \mid x \in \mathbb{R}, \lfloor x \rfloor \text{ is odd}\}$.
\begin{enumerate}[(i)]
\item Draw the real number line from $-5$ to 5, and indicate the sets $R_1, R_2 \in \mathcal{P}$.
\item Graph the associated binary relation $\rel_\mathcal{P}$. You only need to graph from $-5$ to 5.
\end{enumerate}
...
The relation in digraph A is transitive. Notice how transitivity is defined: \emph{if} $a \rel b$ and $b \rel c$ then  $a \rel c$; in other words, if there's an indirect route (one arrow following another) to get from one element to another by going through a third element, then there must also be a direct route (single arrow).  Notice there's an indirect route from 3 to 2 by going through 1, but there's also a direct route from 3 to 2 (or more formally, $3 \rel 1$, $1 \rel 2$, and $3 \rel 2$). Furthermore this is the only example in A of an indirect route. Therefore this relation is transitive. If $3 \rel 2$ is removed from this binary relation, then the relation isn't transitive because it's still be possible to get from 3 to 2 via 1, but there's no longer a direct route.
so 
The relation in digraph B is also transitive. This is true because the \emph{if} clause $\bigl((a \rel b)$ and $(b \rel c)\bigr)$ never holds, since there's no case of an indirect route. In other words, if you can \emph{prove} that there's no counterexample to a mathematical statement, then the statement must be true! Even if $3 \rel 1$ is removed from this relation and the digraph is just three dots, the relation is still transitive.
...
%\section{Equivalence relations OLD} \label{EquivalenceRelationsOLD}
...
\section{Examples of equivalence relations\quad\sectionvideohref{PL2uooHqQ6T7PW5na4EX8rQX2WvBBdM8Qo}}
...
\begin{exercise}{}
Do you see anything that all of the above examples have in common? (\emph{Spoiler Alert}: if you can't think of an answer, just keep reading.)
\end{exercise}

The above equivalence relations can all be defined in terms of \emph{functions}. More specifically, they can all be defined according to the following pattern:

Given a function $f: A \rightarrow B$, define a binary relation on $A$ by: $a_1 \rel a_2$ iff $f(a_1) = f(a_2)$.

\noindent
Definitions of this form seem to always produce equivalence relations. (We should say that in some cases, the definition we provided must be slightly massaged to bring it into this form.  For example, the equivalence relation in Example~\ref{example:EquivalenceRelationsChap:NxNEquivRelEg} follows the pattern if we use the function $f: \mathbb{N} \times \mathbb{N} \rightarrow \mathbb{Z}$ defined by $f(\, (a,b)\, ) = a-b$. )
...
We've seen enough examples to convince us that functions produce equivalence relations. But examples are never enough! The bottom line is that we need a proof--and here it is.
...
\begin{enumerate}[(a)]
\item
Show that $a \rel_\mathcal{P} a \iff f(a)=f(b)$.
\item 
...
\item
We also know that the function $f$ produces an equivalence relation on the domain of $f$, as in Proposition~\ref{proposition:EquivalenceRelationsChap:EquivRelFromFunc}.  Draw a digraph that represetns this equivalence relation.
\item
...
\section{Obtaining partitions from equivalence relations\quad\sectionvideohref{PL2uooHqQ6T7PW5na4EX8rQX2WvBBdM8Qo&index=20}} \label{EquivalenceRelationsDefnSect}
...
\begin{enumerate}[(a)]
\item
Suppose we begin with a partition $\mathcal{P}$ on the set $A$, and define an equivalence relation $\sim_{\mathcal{P}}$ as in Proposition~\ref{proposition:EquivalenceRelationsChap:parteqrel}. Next, suppose that following Proposition~\ref{proposition:EquivalenceRelationsChap:EquivRel->Part} we define a partition   $\mathcal{P}'$  consisting of the equivalence classes of $\sim_{\mathcal{P}}$. Show that $\mathcal{P}=\mathcal{P}'$. 
\item
