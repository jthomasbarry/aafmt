\chap{Group Actions and Regular Polyhedra }{actions}
...
\begin{example}{Action5}
Let $G$ be a group and $H$ be a subgroup of $G$. Let $S=\{xH~|~x\in G \}$ which is the set of all left cosets of $H$ in $G$. Then $G$ acts on $S$ by $g.xH = (gx)H$, which is also a coset of $H$. Show that $G$ is acting on $S$, which means that $S$ is a $G$-set. To check we must show identity and compatibility:
\begin{enumerate}[(1)]
\item Check identity: $e.xH=exH=xH$
\item Check compatibility: Let $a,b \in G$, $(ab).(xH)=(ab)(xH)=(abx)H=a(bxH)=a.(b.(xH))$
\end{enumerate}
Therefore this is an action of $G$ on the set of left cosets $H$.
\end{example}

\begin{exercise}{Action6}
Let $G$ be a group and $\mathcal{E}_n$ be the set of all subsets of $G$ with $n$ elements where $n$ is a positive integer and $n\leq |G|$. Let $S \in \mathcal{E}_n$, meaning $S$ is a subset of $G$ with $n$ elements. Then $G$ acts on $\mathcal{E}_n$ by $(g,S) \rightarrow g.S=\{gs~|~s \in S\}$. Note that $gS$ is a subset of $G$ with $n$ elements.
\begin{enumerate}[(a)]
\item Check the identity condition.
\item Check the compatibility condition.
\end{enumerate}
Parts (a) and (b) verify that $\mathcal{E}_n$ is a $G$-set.
\end{exercise}

To show that something is not an action, or that a set is not a $G$-set, one must show that either the identity or compatibility condition fails in the definition of action and $G$-set.
...
\section{Euler's formula for regular polyhedra}\label{sec:actions:Euler}
...
\section{Are there  other regular polyhedra?}
...
\begin{example}{IntLatIntro} Consider a particular group element $a=(0.7,0.5)$. Use a graphical illustration to model the point $a$ mapping the integer lattice $H$ which results in the coset $a+H$.
...
Add small section defining floor function...

\begin{exercise}{IntLatBijection} Using bijection, show that each coset of the form $a+H$, where $a\in\mathbb{R}^2$, has a single point, $a+h$ for some $h\in H$, inside the unit square.
\end{exercise}

\begin{exercise}{IntLatGset} Show that the set of all cosets of the form $\{a+H, x \in \mathbb{R}^2\}$ is a $\mathbb{R}^2$-set. (I THINK I want this question...need to think on it more...it's just a placeholder for now)
\begin{enumerate}[(a)]
\item walk students through determining what the . is in g.a and how to show that it is a r2 set
\item hjkl
\end{enumerate}
\end{exercise}

\begin{example}{IntLat1} Continuing from Example~\ref{example:actions:IntLatIntro}: let an additional group element be $b=(0.8,0.3)$, where $b \in G$. Find the point $h=(m,n) \in H$ such that $b+a+h$ is inside the unit square (recall $a=(0.7,0.5)$).
...
\end{align*}
If $m=-1$ and $n=0$, then $b+a+h=(1.5-1,0.8+0)=(0.5,0.8)$, so $h=(-1,0)$. Generally the point $h=(m',n')$, where $m'=floor(x_1)$ and $n'=floor(x_2)$. ***FIX***
\end{example}

The point $b+a+h=(0.5,0.8)$ is the \emph{only} point of the coset $b+a+H$ which is inside the unit square. By basic properties of cosets, it follows that $b+a+H =(0.5, 0.8)+H$. Recall that by definition a left coset of the integer lattice $x+H$ means adding the same point, $a=(x,y)$, to each point in the integer lattice, $H$. A group action simply changes one coset of $H$ in $G$ to a different coset.  The illustrations in Figure~\ref{fig:IntegerLattice1} show the movement of the coset $a+H$ when acted on by $b$ on the left from Example~\ref{example:actions:IntLat1}. 

\begin{figure}[htpb]
\begin{center}
\includegraphics[width=4.0in]{images/Lattice_baH.png}
\caption{Illustrating  the movement of the coset $a+H$ for $a=(0.7,0.5)$ when acted on by $b=(0.8,0.3)$ on the left from Example~\ref{example:actions:IntLat1}. The lattice, $H$ is represented by blue points, the elements of the coset $a+H$ are represented by red diamonds, and the elements of the final coset $b+a+H$ are represented by purple crosses.}
\label{fig:IntegerLattice1}
\end{center}
\end{figure}

In Example~\ref{example:actions:IntLat1} we saw $b$ acting on the coset $a+H$ producing the coset $b+a+H$. What is an equivalent coset $c+H$, where $c \in G$, such that $c+H=b+a+H$? To do this simply add $b+a=(0.7,0.5)+(0.8,0.3)=(1.5,0.8)=c$. Notice the coset $c+H$ has a unique element within the unit square, namely $(0.5,0.8)$ which is equal to $c+(-1,0)$ (we'll make use of this point in the next paragraph).

\begin{figure}[htpb]
\begin{center}
\includegraphics[width=4.0in]{images/Lattice_cH.png}
\caption{Illustrating the coset $c+H$ where $c=b+a$ from Example~\ref{example:actions:IntLat1}. The lattice, $H$ is represented by blue points, the elements of the coset $a+H$ are represented by red diamonds, and the elements of the final coset $b+a+H=c+H$ are represented by purple crosses.}
\label{fig:Lattice_cH}
\end{center}
\end{figure}

Example~\ref{example:actions:IntLat1} can be used to describe the motion of a character on the screen of a ``wraparound'' video game.  Imagine the unit square is your TV or computer screen. Suppose the character starts near the right edge at the point $a=(0.7,0.5)$, and undergoes linear motion to the right and up corresponding to the displacement vector $b=(0.8,0.3)$ then he moves off the right edge of the screen and re-appears at the left edge, ending up at the point $(0.5,0.8)$ as shown in Figure~\ref{fig:wraparound1}. 
...
Let's have a little more fun! In the next example actions on H will be modeled on $\mathbb{R}^2:[-2,2] \times [-1,3]$ so that you will understand what is only visible on the unit square as $g$ acts on $x+H$

\begin{example}{IntLat2} Consider group elements $a=(-0.6,-0.4)$ and $b=(0.5,1.3)$ where $a,b \in \mathbb{R}^2$.
\begin{enumerate}[(a)]
\item Find the point $h=(m,n) \in H$ such that $b+a+H$ is inside the unit square.
\item Find $c$ in the unit square, such that $c+H=b+a+H$.
\item Graph the path of the points that are visible to only the unit square throughout the group action $b$ on $a+H$. (We will use ``zoomed out" graphs initially to help illustrate the paths to graph only on the unit square)
\end{enumerate}
\end{example}

\begin{exercise}{IntLatHW1}
\begin{enumerate}[(a)]
\item Given $a=(0.8,0.6)$ and $b=(1.4,0)$ find the point $h\in H$ such that $b+a+h$ is inside the unit square. (In fact, this is the \emph{only} point $h$ for which $b+a+H$ is inside the unit square.)
\item Given $a=(0.8,0.6)$ and $b=(1.2,1.3)$ find the point $h\in H$ such that $b+a+h$ is an element of the unit square.
\item Given $a=(0.8,0.6)$ and $b=(0,3.5)$ find the point $h\in H$ such that $b+a+h$ is inside the unit square.
\item For each $a$ and $b$ above, find $c$ in the unit square, such that $c+H=b+a+H$.
\item Illustrate part (a) with a graph.  Graph the point $a$.  (This is actually the point $x+h'$ for $h'=(0,0)$). Then graph the point $b+a+h$ using the $h$ you found in part (a).  Include ordered pairs to indicate the position of points. Include arrows to indicate the apparent ``movement'' from the initial point to the 
final point within the unit square (similar to Figure~\ref{fig:UnitSquare}.
\item Create a similar graphs illustrating part (b) and (c).
\item Prove that the above are examples of a group action of $G$ on cosets of the form $\{a+H, a \in \mathbb{R} \times \mathbb{R}$.  That is, show that the set of cosets of the form $a+H$ is a $G$-set. 
\end{enumerate}
\end {exercise}
