\chap{Groups Actions and Regular Polyhedra }{actions}
...
\begin{defn}
Let $X$ be a set and $G$ be a group. A \term{(left) action}\index{Actions!group} of $G$ on $X$ is a map $G\times X\rightarrow X$
given by $(g, x)\rightarrow gx$, where
\begin{enumerate}[(1)]
\item $ex = x$ for all $x\in X$;
\item $(g_1g_2)x = g_1(g_2x)$ for all $x\in X$ and all $g_1, g_2 \in G$.
\end{enumerate}
The set $X$  on which $G$ acts is called a  \term{$G$-set}.\index{G-set@$G$-set}
\end{defn}
...
Let $G$ be the $xy$-plane under addition (that is, $G=(\mathbb {R}^2,+)$).  Let $H=\mathbb{Z}\times \mathbb{Z}$, which is a subgroup of $G$ ($H$ is called the \term{integer lattice}\index{Integer!lattice}).  
...
the right edge at the point $x=(0.9,0.5)$, and undergoes linear motion to the right corresponding to the displacement
...
The rectangles should be thought of as unit squares (the scales on the $x$ and $y$ axes are not equal).   These figures illustrate a left group action of $(\mathbb{R},+)$ on a left coset of integer lattice. The lattice is represented by blue points, while the elements of the coset are represented by red diamonds.}
...
\begin{enumerate}[(a)]
\item Given $x=(0.8,0.6)$ and $g=(1.4,0)$ find a point  $h\in \mathbb{Z}\times \mathbb{Z}$ Such that $g+x+h$ is inside the unit square $[0,1) \times [0,1)$.  (In fact, this is the \emph{only} point $h$ for which $g+x+H$ is inside the unit square.)
\item Given $x=(0.8,0.6)$ and $g=(1.2,1.3)$ find a point  $h\in \mathbb{Z}\times \mathbb{Z}$ Such that $g+x+h$ is an element of the unit square.
...
\item Illustrate part (a) with a graph.  Graph the point $x$.  (This is actually the point $x+h'$ for $h'=(0,0)$).  Then graph the point $g+x+h$ using the $h$ you found in part (a).   Include ordered pairs to indicate the position of points.  Include arrows to indicate the apparent``movement'' from the initial point to the 
...
\item Prove that the above are examples of a group action of $G$ on cosets of the form $\{x+H, x \in \mathbb{R} \times \mathbb{R}$.  That is, show that the set of cosets of the form $x+H$ is a $G$-set. 
\end{enumerate}
\end {exercise}
