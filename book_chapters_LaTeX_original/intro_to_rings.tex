...
We've already mentioned that ${\mathbb Q}$ is a ring. Sometimes we can use rings to create larger rings by adding additional elements. Such rings are called \term{extension rings}. We will discuss these further in Section~\ref{sec:Rings:ExtensionRings}. For now, let's look at a particular example.
...
In Section \ref{sec:Rings:DefinitionsAndExamples}, we learned that ${\mathbb C}$, the set of all complex numbers, forms a ring.  Consider the following subset of ${\mathbb C}$:
...
We may recall the definition of \term{partition} from Section \ref{sec:EquivalenceRelations:PartitionsAndProperties} 
...
%%% Definition \ref{definition:Rings:defideal}
