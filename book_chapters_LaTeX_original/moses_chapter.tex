...
%%%%(c)
%%%%(c)
\chap{Further topics in Cryptography: Diffie-Hellman Key Exchange and Elliptic Curve Cryptography}{Crypto}


In Chapter~\ref{crypt} we introduced basic notions of cryptography. We also discussed a few examples of cryptosystems, beginning with simple shift codes (pretty easy to crack) and progressing up to RSA (not crackable with current technology).  But what we've seen so far is just the beginning of the subject. In this chapter we'll explore two more topics. First we'll look at Diffie-Hellman key exchange, which is used to set up cryptosystems for further communication. then, we'll look at elliptic curve cryptography, which is a new class of cryptosystems first discovered in 1985@@@Write a 'historical note' to put at the end of the chapter@@@. 

\section{Diffie-Hellman Key Exchange}\label{sec:DHKE:1}

In order to share a private message over a public domain a sender must  "lock" or encrypt their message using a \emph{key}\index{key! in cryptogaphy}.  Recall from Section~\ref{sec:crypt:overview}, in cryptography a  key is a special piece of information (usually a number)  that is required to encrypted and decrypt data that is shared between the sender and receiver. There are generally three types of keys used: public, private and symmetric. A \term*{Public Key}\index{Public Key! in cryptography} can be widely distributed, and is used for encrypting messages.  A\term*{Private Key}\index{Private Key! in cryptography} is only known to the sender, and is usually used for decrypting messages (but can also be used for encrypting messages in some cryptosystems, such as RSA).  Finally, a \term*{Symmetric Key}\index{Symmetric Key! in cryptography} is known by the sender and receiver, and is used to encypt and decrypt messages.   

Symmetric keys are convenient to use for two-way communication. But before communicating, the two users must find a way to secretly decide on a common key. This is not always so easy. Suppose the only way the users have to communicate is via a public network, where eavesdroppers can listen in to whatever information they're sending to each other. Would it even be possible to establish a common, secret key in such a situation?  Amazingly, it is-- and that's what Diffie-Hellman Key Exchange (DHKE) is all about.  As we shall see, tDHKE is a public key algorithm that is used to establish a secret symmetric key between two parties over an unsecure network.  Note that the DHKE is not used for sending messages, but only to establish a key that can be used to encrypt/decrypt subsequent messages.  

How does DHKE work? Let's suppose Moses and Rachael want to establish a symmetric key over a public communication channel. They may follow the  steps in the DHKE algorithm  as follows (see also Figure~\ref{fig:DH:DHKE_1}):
...
\item Moses chooses a secret integer $n$, known only to himself.  He then computes $q$ where $q =\mod(g^n,p)$, and sends Rachael the value of $q$;
...
Fill in the blanks in the following proof that  $K_1$ is always equal to  $K_2$.\\
  	\begin{proof} 
		...
		K_1 &=   \bmod (r^n , \underline{~~~~~~}) 
           	\\&=  \bmod ( \underline{@@@change here@@@}^{\underline{~~~~~~}} , p) , p )&& \text{(substitution)}	%ans: \bmod ( \bmod (g^m , p)^n , p )&& \text{(substitution)}
...
In order to make things clearer, we give a simple example with actual numbers. This example is much too simple to be used in practice, but it does at least show the procedure.
...
\end{enumerate}
\end{eg}
It is important to choose a large prime number $p$, in order to make it difficult for an eavesdropper to crack your code (we will explain why this is true a little later).  Computers use a base-2 number system known as the binary number system.  Internet protocols usually use a minimum of a 1024-bit prime number for $p$, so that $2^{1023} < p < 2^{1024}$  (to get an idea of how big this is, to write $p$ out in decimal would take about 300 digits).  So, there are $2^{1023}$ different possibilities for 1024 bits (where the highest bit is a `1'), but how can you guarantee that the number is prime?
...
\end{enumerate}
\end{exer}
Recall that the logarithm is the inverse operation of exponentiation.  For example, suppose $a = g^m$ , then the Discrete Logarithm (DL) of $a$ = $m$ and is expressed as    $\log_g a = m$.  The security of the DHKE leverages the easy computations of exponentials versus the difficulty of computing logarithms. 
