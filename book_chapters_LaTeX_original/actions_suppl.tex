...
\begin{enumerate}[(a)]
\item 
%Let $G=\{\var{id},\mu_1\}$ which is a subgroup of $S_3$ (the symmetry group of an equilateral triangle) (See Figure~\ref{fig:groups_s3_symmetry} in Section~\ref{sec:Symmetry:Group}.) Let $X=\{A,B,C\}$ be the set of vertices of an equilateral triangle. List the orbits of $X$ under $G$
%\\
%\\
...
\item 
%Prove your equation using Proposition~\ref{proposition:GroupActions:CountingFormula}.
...
\begin{enumerate}[(a)]
\item
%Using Proposition~\ref{proposition:GroupActions:CountingFormula}, find the number of edges.
By Proposition \ref{proposition:GroupActions:CountingFormula}, $|{\cal O}_x| = [G : G_x]$.  
\\
This can be rewritten as $|{\cal O}_x| = \frac{|G|}{|{G_x|}}$, by Definition \ref{definition:Cosets:factor_group}.
\\
...
\item
%Using Proposition~\ref{proposition:GroupActions:CountingFormula}, find the number of edges which meet at each vertex.
