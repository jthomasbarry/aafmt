...
%\\
Proof: Given a finite group with $g \in G, h \in G$ such that $|g| = 5, |h| = 7$, then by Theorem~\ref{OrbitIsSubgroup}, $\langle g \rangle$ is a subgroup of $G$.
\\
...
\\
Also by Theorem~\ref{OrbitIsSubgroup}, $\langle h \rangle$ is a subgroup of $G$.
\\
...
%\\
\\
Theorem~\ref{LagrangeTheorem} tells us that $G$ is evenly divided by $H$ and we know that $|G| = 60$.
\\
...
%\\
\\
Let gcd$(a, p) = 1$.  Then we know that $a^{\phi(p)} \equiv 1 \pmod{p}$, by Theorem~~\ref{cosets:Eulers_theorem}.
\\
...
\begin{align*}
gHg^{-1} \subset H \text{\ and\ } gHg^{-1} &= H &\text{(by Theorem~\ref{normal:normalequivalents})}
\\
ghg^{-1} &= h' &h' \in H
\\
